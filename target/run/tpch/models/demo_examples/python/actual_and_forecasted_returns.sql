  ...
  create or replace   view sa_mark_wan.dbt_c_mwan.actual_and_forecasted_returns
  
   as (
    with agg_daily_returned_orders as (
    select * from sa_mark_wan.dbt_c_mwan.agg_daily_returned_orders
),

forecast_daily_returns as (
    select * from sa_mark_wan.dbt_c_mwan.forecast_daily_returns
),
