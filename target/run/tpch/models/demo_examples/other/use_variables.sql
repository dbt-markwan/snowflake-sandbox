  ...
  create or replace   view sa_mark_wan.dbt_c_mwan.use_variables
  
   as (
    -- This is here to show that data older than start_date exists - run this first
-- select min(order_date) from sa_mark_wan.dbt_c_mwan.fct_order_items
...
select * from sa_mark_wan.dbt_c_mwan.fct_order_items where order_date >= '1999-01-01'
  );
